...
function ThreeActCard({ feature }: { feature: FeatureData }) {
  ...
  return (
    <div
      className={`feature-card glass-card ${isResult ? "fc-result" : isPain ? "fc-pain" : "fc-action"}`}
    >
      {/* Act indicator strip */}
      ...
      {/* Metric widget */}
      <div
        className={`metric-widget ${isPain ? "mw-pain" : isResult ? "mw-result" : "mw-action"}`}
      >
        {isPain && (
          <>
  ...
  return (
    <div className="mini-console font-mono">
      {logs.map((line, i) => (
        <div
          key={i}
          className={`mc-line ${line.startsWith("✓") ? "mc-ok" : line.startsWith("  ↳") ? "mc-info" : "mc-cmd"}`}
        >
          {line}
        </div>
      ))}
